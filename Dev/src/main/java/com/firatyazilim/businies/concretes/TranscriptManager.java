...
public Result deleteById(int transId) {
	Transcript transcript=this.repository.getById(transId);
	if(transcript==null) return new ErrorResult("Transcript bulunamadı");
	else {this.repository.deleteById(transId);
